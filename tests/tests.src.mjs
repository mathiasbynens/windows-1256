...
import * as windows1256 from '../windows-1256.mjs';

console.log('Testing `windows1256.encode`…');
...
assert.deepStrictEqual(
	windows1256.decode(new Uint16Array([0xFFFF]), { mode: 'replacement' }),
	'\uFFFD',
...
assert.throws(
	() => {
		windows1256.decode(new Uint16Array([0xFFFF]), { mode: 'fatal' });
	},
	Error,
...
console.log('Testing `windows1256.labels`…');
assert.ok(Array.isArray(windows1256.labels));
assert.ok(windows1256.labels.length > 0);
